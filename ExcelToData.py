...
for row in worksheet.iter_rows(min_row=3, min_col=3, max_col=3):
    ...
    for cell in row:
        ...
        if current_value == previous_value:
            ...
            # 将D列和F列的值保存到字典中
            #data_dict.setdefault(current_value, {}).setdefault('科目'+str(i), )
            data_dict.setdefault(current_value, {}).setdefault('成绩',{}).setdefault(d_column_value, f_column_value)
            #data_dict.setdefault(current_value, {}).setdefault(d_column_value, f_column_value)
            #data_dict[current_value]['成绩']= {d_column_value, f_column_value}
            ...
            # 将D列和F列的值保存到字典中
            data_dict.setdefault(current_value, {}).setdefault('成绩',{}).setdefault(d_column_value, f_column_value)
            #data_dict.setdefault(current_value, {}).setdefault('成绩'+str(i), f_column_value)
...
for key, value in data_dict.items():
    #print(f"Value '{key}' has D value: {value['科目1']} and F value: {value['成绩1']}")
    print(data_dict)
